...
public class Term {
	...
	private double _coef; // = 2.1 coefficient
	...
	public Term(String s) throws PolyException {
		...
		String[] factors = s.split("\\*"); //splits by multiplication
		for (String factor : factors) {
			...
			try {
				...
				_coef *= Double.parseDouble(factor); 		//multiply factor by coef for each factor in the term			
				...
				String[] var_pow = factor.split("\\^"); //split by exponentiation
	...
	//to be able to ensure the coef is not 0.0d, we need to create a method to get the coef in other classes (it is private to this class)
	public double getCoef () {
		return _coef;
	}
	
	
	/** If Term defines a function f(x,y) = 2xy^2 and assignment is { x=2.0 y=3.0 } 
	...
	public double evaluate(Vector assignments) throws Exception {
		...
		double evaluation = _coef;
		
		for (String var : _vars) {
			int index = _vars.indexOf(var); //find at what index the var is located, so when we use the method 'get',
											//we can use that index to find the power of the var
			evaluation *= Math.pow(assignments.getValue(var), _pows.get(index));
		}
		
		return evaluation;
	}

	//to be able to return a new term in method 'differentiate', we need to be able to copy the old term into the new term
	public Term (Term oTerm) { //take original term as parameter for copying
		this(oTerm._coef); //take coefficient of oTerm
		_vars = new ArrayList<String> (oTerm._vars); //take vars of oTerm
		_pows = new ArrayList<Integer> (oTerm._pows); //take powers of oTerm
	}
	...
	public Term differentiate(String var) throws Exception {
		...
		Term pD = new Term (this); 
		int index = this._vars.indexOf(var);
		
		if (!this._vars.contains(var)) { //partial derivative will be 0 if the var is not in the term
			return new Term(0);
		}

		else if (this._coef ==0 || this._vars.isEmpty() || this._pows.isEmpty()) { //exceptions
			return null;
		}
		
		else if (this._pows.get(index)==1) { //another special case: derivative will result in the value of var to be 1 and for the var to no longer show up in the term
			pD._vars.remove(index);
			pD._pows.remove(index);
		}
		
		else { //in the differentiation of the var, it becomes coef*p*x^(p-1)
			pD._coef = this._coef*this._pows.get(index); //coef*p is the new coef
			int p = _pows.get(index); //current power
			pD._pows.set(index, p-1); //now we set the new power to p-1
		}
		return pD;
	}
}
