...
public class Minimizer {
	...
	public void setX0(Vector x0) throws Exception { 
		_x0.clear();
		if (x0 != null) {
			_x0.setAll(x0);
		}
	...
	public void minimize(Polynomial p) throws Exception {
		...
		_var2gradp = new HashMap<String,Polynomial> (); //make new hashmap
		...
		while (getNIter() < getMaxIter() && getLastGradNorm() > getEps()) {
			_nIter++;
			
			Vector grad = new Vector();
			for (String var : _var2gradp.keySet()) { //for each var in all the vars
				grad.set(var, _var2gradp.get(var).evaluate(getLastPoint())); //set grad to have the value of the gradient for that var
			}
			
			_lastGradNorm = grad.computeL2Norm(); //compute gradient norm
			_lastx = getLastPoint().sum(grad.scalarMult(-1*getStepSize())); // compute new point by summing current point and the negation of the step size times the gradient
			//	  evaluate the objective at the new point
			_lastObjVal = p.evaluate(getLastPoint()); //evaluate the objective at the new point
			System.out.format("At iteration %d: %s objective value = %.3f\n", _nIter, _lastx, _lastObjVal); 
		}
